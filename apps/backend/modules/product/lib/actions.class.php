 ...
 */
abstract class autoProductActions extends sfActions
{
  ...
  public function executeFilter(sfWebRequest $request)
  {
    ...
    if ($request->hasParameter('_reset'))
    {
      ...
      $this->redirect($this->helper->getIndexRoute());
    }
...
//	$form->getObject()->refresh();
    ...
    if ($form->isValid())
    {
      ...
      if ($request->hasParameter('_save_and_add'))
      {
        ...
        $this->redirect($this->helper->getNewRoute());
      }
      else
      {
        $this->getUser()->setFlash('notice', $notice);

        $this->redirect(array('sf_route' => $this->helper->getEditRoute(), 'sf_subject' => $product));
      }
    }
    else
    {
  ...
  {
    return Doctrine::getTable('Product')->hasColumn($column);
  }
}
