...
class imageTransform {
	...
	var $cache = true; // Cache folder
	var $expire = 604800; // Cache expiration time in seconds
	...
	*/
	function view ($mode, $image, $param, $cache = true) {
		...
		if ($cache && $this->cache) {
			...
			$this->cache = dirname($image) . '/cache/';
			...
			if (!$this->recursive_path($this->cache) || !is_writable($this->cache)) {
				return $this->debug('Can\'t write in the cahe folder1: '.$this->cache);
			}
