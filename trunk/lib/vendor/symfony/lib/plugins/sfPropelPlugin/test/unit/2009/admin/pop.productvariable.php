 ...
 */
  ...
  if (!$boolClose) {
    ...
    if (@in_array($_GET["action"], array("update", "insert"))) {
      if ($arrGlobalRights[5475] == "write") 
      {
        include(MODULE_DIR . "productcatalog/actions.variable.inc");
      }
      else 
       {
        $_GET["action"] = "view";
  ...
  $engSmarty->assign("recProductVariable", $recProductVariable);
  $engSmarty->assign("arrTranslation", $arrTranslation);
  $engSmarty->display("admin/pop.productvariable.tpl");
?>
