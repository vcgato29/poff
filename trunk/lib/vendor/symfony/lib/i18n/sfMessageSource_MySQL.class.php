...
class sfMessageSource_MySQL extends sfMessageSource_Database
{
  ...
  {
    $variant = mysql_real_escape_string($variant, $this->connect());
	$this->connect();
  $statement =
  ...
  {
    $source = mysql_real_escape_string($source, $this->connect());

    $rs = mysql_query("SELECT date_modified FROM catalogue WHERE name = '{$source}'", $this->connect());
  ...
  public function isValidSource($variant)
  {
    ...
    $rs = mysql_query("SELECT COUNT(*) FROM catalogue WHERE name = '{$variant}'", $this->connect());
  ...
  protected function getCatalogueDetails($catalogue = 'messages')
  {
    ...
    $name = mysql_real_escape_string($this->getSource($variant), $this->connect());

    $rs = mysql_query("SELECT cat_id FROM catalogue WHERE name = '{$name}'", $this->connect());
    ...
    $rs = mysql_query("SELECT COUNT(*) FROM trans_unit WHERE cat_id = {$cat_id}", $this->connect());
  ...
  protected function updateCatalogueTime($cat_id, $variant)
  {
    ...
    $result = mysql_query("UPDATE catalogue SET date_modified = {$time} WHERE cat_id = {$cat_id}", $this->connect());
  ...
  function save($catalogue = 'messages')
  {
    ...
    foreach ($messages as $message)
    {
      $count++;
      $inserted++;
      $message = mysql_real_escape_string($message, $this->connect());
      $statement = "INSERT INTO trans_unit
        (cat_id,id,source,date_created) VALUES
        ({$cat_id}, {$count},'{$message}',$time)";
	
      
      mysql_query($statement, $this->connect());
    }
    if ($inserted > 0)
    {
  ...
  function delete($message, $catalogue = 'messages')
  {
    ...
    $text = mysql_real_escape_string($message, $this->connect());
    ...
    if (mysql_affected_rows($this->connect()) == 1)
    {
  ...
  function update($text, $target, $comments, $catalogue = 'messages')
  {
    ...
    $comments = mysql_real_escape_string($comments, $this->connect());
    $target = mysql_real_escape_string($target, $this->connect());
    $text = mysql_real_escape_string($text, $this->connect());
  ...
  function catalogues()
  {
    ...
    $rs = mysql_query($statement, $this->connect());
    $result = array();
    while($row = mysql_fetch_array($rs, MYSQL_NUM))
    {
